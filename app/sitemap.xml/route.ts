...
export async function GET(req: Request) {
  ...
  const isMainDomain = isLocal || isVercel || hostname === (process.env.NEXT_PUBLIC_APP_URL ? new URL(process.env.NEXT_PUBLIC_APP_URL).hostname : "");
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  // Fallback or Main Domain Case: Serve a general sitemap or a list of all clinics
  if (!sitemapXml) {
    try {
